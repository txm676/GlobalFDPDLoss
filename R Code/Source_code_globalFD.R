...
if (length(extant$species[which(!extant$species %in% sayol$species)]) != 7) stop("wishbone A") 

##check species in both and remove:
#there are seven species listed in both. Six are EW and we will class these as extinct
#The 7th (Zosterops conspicillatus) has recently been split and the nominate subpsecies
#i.e. this name, is now considered extinct with the other subspecies extant and given a new
#name (Z. saypani). As such, remove all seven from AVONET.
...
extinct$ExtinctionPeriod[which(extinct$ExtinctionPeriod == "EW")] <- "EH"
extinct$ExtinctionType[which(extinct$ExtinctionType == "EW")] <- "EH"
...
#and anthropogenic (EP) extinctions. For current species, just say "Current"
...
if (!is.null(ind_impu)){
  ...
  if (!nrow(ind_impu2) == 600) stop("arctic")
...
extinct3 <- extinct2
extinct3$species[which(extinct2$species == "Hypotaenidia_owstoni")] <- "Gallirallus_owstoni"

if (!all(extinct3$species %in% phylo_all[[1]]$tip.label)){
  stop("Pneuma")
}

#Note there are 12 species in our extinct dataset that are in Jetz; and then the
#13th in Gallirallus_owstoni. Note, it is 9992 and not 9993 as H owstoni is removed
if(!length(phylo_all[[1]]$tip.label) == 9992 + nrow(extinct2) - 12){
  stop("scar tissue")
}
...
allSp_phylo$ExtinctionPeriod[which(allSp_phylo$ExtinctionPeriod == "EW")] <- "EH"
allSp_phylo$ExtinctionType[which(allSp_phylo$ExtinctionType == "EW")] <- "EH"
...
traits <- dplyr::select(allSp, "Beak.Length_Culmen", "Beak.Length_Nares", 
                        "Beak.Width", 
                        "Beak.Depth", "Tarsus.Length",
                        "Wing.Length", "Kipps.Distance","Tail.Length", "Mass")
...
#allSp$species[allSp_hWI]#should be 20
if (length(allSp_hWI) != 20 | 
...
perc_change <- function(y){
  ...
  r1 <- perc_change_int(v1 = y[1], v2 = y[2])
  r2 <- perc_change_int(v1 = y[1], v2 = y[3])
  r3 <- perc_change_int(v1 = y[2], v2 = y[3])
...
##Get ES and Z-scores for each null distribution
...
##Set TRAIT to one of Mass or Kipps.Distance or Hand.Wing.Index
##set GEOG to one of All, "Yes (for Isl endemic) or "No (for Continental)
##Set TEST to one of Median or SD (for plot A and the null models)
#n = number of null iterations
#titl = ggplot title
#bandwidth = geom violin bw argument; default is 9999 which then uses nrd method
#color = violin colors
...
null_plot <- function(prehistoric_all_F = prehistoric_all,
  ...
  ##extinction numbers
  Hist_N <- table(dat_ind$ExtinctionPeriod)["EH"] %>% as.vector()
  Pre_N <- table(dat_ind$ExtinctionPeriod)["EP"] %>% as.vector()
  All_N <- Hist_N + Pre_N
  if (GEOG == "All"){
  ...
  obs_H <- filter(dat_ind, ExtinctionPeriod %in% c("EH", "Current")) %>%
    dplyr::select(TRAIT) %>%
    summarize(Median = median(!!rlang::sym(TRAIT)), #uses rlang to allow a character be used instead of colname
              SD = sd(!!rlang::sym(TRAIT)))
  ...
  obs_null <- bind_rows(obs_H, obs_C, obs_C, obs_F) %>%
    mutate(Type = c("P2H", "P2C", "H2C", "C2F")) 
  colnames(obs_null)[1:2] <- c("Median", "SD")
  obs_null$Type <- factor(obs_null$Type, 
                          levels = c("P2C", "P2H", "H2C", "C2F"))
  
  ##run null models
  #set n for one of the three scenarios: preH to historic, preH to current, and historic to current
  P2H <- replicate(n, ind_null(dat = dat_ind, trait = TRAIT, n = Pre_N)) %>%
    t() %>%
    as.data.frame()
  P2H$V3 <- "P2H"
  ...
  datH2c <- filter(dat_ind, ExtinctionPeriod %in% c("EH", "Current"))
  ...
  null_all <- bind_rows(P2H, P2C, H2C, C2F)
  ...
  for (i in 1:length(obs_null$Type)){
    ...
    #Kipps is two-tailed test, but others one-tailed
  ...
  if (hist){
    joris <- ggplot(data = null_all, 
     ...
     jorisA <- ggplot(data = null_all, aes(Type, !!rlang::sym(TEST))) + 
      theme_bw() + xlab("Time Period") + ylab(yl) + 
      geom_violin(aes(fill = Type), bw = bandwidth)  +
      scale_fill_grafify(palette = "fishy") +
      geom_point(data = obs_null, aes(Type, !!rlang::sym(TEST)), size = 5,
                 shape = 18, col =  beak_col)  + ggtitle(titl) + 
      theme(axis.text = element_text(size = 13), axis.title = element_text(size = 15),
            plot.title = element_text(size = 17)) +
      theme(legend.position = "none") + 
       scale_x_discrete(labels=c("P2C" = expression(P%->%C),
                                 "P2H" = expression(P%->%H),
                                 "H2C" = expression(H%->%C),
                                 "C2F" = expression(C%->%F)))
     
    #red diamonds are observed median body mass for historic and current periods;
    #and blue is the null median values following the three situations
    
    ##make small version just of the three observed values for inset plot
    obs_null2 <- bind_rows(obs_P, obs_H, obs_C, obs_F) %>%
      mutate(Type = c("P", "H", "C", "F")) 
    ...
    obs$Type <- factor(obs$Type, levels = c("P", "H", "C", "F"))
    
    obs_null <- data.frame("Alpha" = c(obs["H","Alpha"], obs["C","Alpha"], 
                                       obs["C","Alpha"], obs["F","Alpha"]),
                           Type = c("P2H", "P2C", "H2C", "C2F")) 
    obs_null$Type <- factor(obs_null$Type, 
                            levels = c("P2C", "P2H",  "H2C", "C2F"))
    ...
    null_all$Type <- rep(c("P2H", "P2C", "H2C", "C2F"), (nrow(null_all) / 4))
    null_all$Type <- factor(null_all$Type, 
                            levels = c("P2C", "P2H", "H2C", "C2F"))
    ...
    jorisA <- ggplot(data = null_all, aes(Type, Alpha)) + 
      ...
      geom_violin(aes(fill = Type), bw =  "nrd")  +
      scale_fill_grafify(palette = "fishy") +
      geom_point(data = obs_null, aes(Type, Alpha), size = 5,
                 shape = 18, col =  beak_col)  + ggtitle(titl) + 
      theme(axis.text = element_text(size = 13), axis.title = element_text(size = 15),
            plot.title = element_text(size = 17))+
      theme(legend.position = "none") + 
...
int_null3 <- function(methodz, mat_beak_dat, trait_null_dat = NULL, 
  ...
  if (methodz == "hyper"){
    ...
    #using picante::pd for now as BAT takes much longer and uses more memory
      #if phylogeny used, include the root, but nj dendrogram is unrooted
...
#####methods. If a null run, first randomly set N specie extinct###########
######################################################################

#n4 = number of species to go extinct
#rn4 = rowname for PA matrix (e.g. P2H, H2C)
...
int_null2 <- function(dat_ind, prehistoric_F, historic_F, current_F, 
  ...
  ##P2H
  BAT_beak_null_H <- int_null4(dat_ind4 = prehistoric_F, n4 = Pre_N, 
                               rn4 = "P2H", method = method, 
                               axes_F = axes_F,
                               method.hv = method.hv, 
                               svmg = svmg, BAND = BAND,
  ...
  if (method == "tree"){
    ...
    BAT_beak_null_H_dispersion <- BAT_beak_null_H[[2]]
    BAT_beak_null_H <- BAT_beak_null_H[[1]]
  ...
  if (method == "hyper"){
    if (analysis == "main"){
    beak_r <- matrix(NA, nrow = 4, ncol = 2)
    colnames(beak_r) <- c("Alpha", "Dispersion")
    
    
    beak_r[,2] <- c(as.vector(BAT::kernel.dispersion(BAT_beak_null_H, 
                                                     func = "divergence", frac = 1)),
                    as.vector(BAT::kernel.dispersion(BAT_beak_null_C, 
                                                     func = "divergence", frac = 1)),
      ...
      beak_r <- matrix(NA, nrow = 4, ncol = 1)
      colnames(beak_r) <- c("Alpha")
    
    }
    
    #Alpha values in both main and beak
    beak_r[,1] <- c(as.vector(BAT::kernel.alpha(BAT_beak_null_H)),
                    as.vector(BAT::kernel.alpha(BAT_beak_null_C)),
    ...
    beak_r <- matrix(NA, nrow = 4, ncol = 2)
    colnames(beak_r) <- c("Alpha", "Dispersion")
    beak_r[,1] <- c(BAT_beak_null_H, BAT_beak_null_C, 
    ...
    beak_r[,1] <- c(BAT_beak_null_H, BAT_beak_null_C, 
                    BAT_beak_null_HC, BAT_beak_null_CF)
    beak_r[,2] <- c(BAT_beak_null_H_dispersion, BAT_beak_null_C_dispersion, 
                    BAT_beak_null_HC_dispersion, BAT_beak_null_CF_dispersion)
  }
  
  rownames(beak_r) <-  c("P2H", "P2C", "H2C", "C2F")
...
null_hyper_ZP <- function(beak_r_obs, hurricane, plot_null = FALSE){
  ...
  resZP_beak <- matrix(nrow = 4*ncol(beak_r_obs), ncol = 6)
  colnames(resZP_beak) <- c("Z", "P", "Metric", "Period", "SES", "SES_P")
  k <- 1
  #set up new version of observed values with H as first row 
  #and then two currents and then F1 and F2
  #if method = hyper, don't do it for centroid (so not third column)
  ZP_obs <- rbind(beak_r_obs[c("H", "C"), c("Alpha", "Dispersion")], 
                  beak_r_obs[c("C", "F"), c("Alpha", "Dispersion")])
  
  rownames(ZP_obs) <- c("P2H", "P2C", "H2C", "C2F")
  
  #if plot_null, create a list to save the ggplots
  if (plot_null){
    plot_null_list <- vector("list", 
                             length = ncol(beak_r_obs) * 4)
  }
  ...
  for (i in 1:4){
    ...
    for (j in 1:ncol(beak_r_obs)){
      ...
      ob_ZP <- ZP_obs[M2, M]
      resZP_beak[k, 1:2] <- round(zP(dum_ZP, ob_ZP, type = "ES"), 2)
      resZP_beak[k, 3] <- M
      resZP_beak[k, 4] <- M2
      resZP_beak[k, 5:6] <- round(zP(dum_ZP, ob_ZP, type = "SES"), 2)
      k <- k +1
    }#eo j
  }#eo i
...
# rn = number of replications to do for the observed hypervolume values (mean then taken)
...
null_hyper <- function(allSp2_F = allSp2, 
  ...
  if (GEOG == "All"){
    ...
    historic_F <- allSp2_F %>% 
      filter(ExtinctionPeriod %in% c("EH", "Current")) 
    current_F <- allSp2_F %>% 
  ...
  if (observed){
    ...
    ##Prehistoric
    BAT_beak_obs_P <- int_null4(dat_ind4 = prehistoric_F, rn4 = "P", 
    ...
    ##historic
    BAT_beak_obs_H <- int_null4(dat_ind4 = historic_F, rn4 = "H", 
                                repn = rn, method = method, 
                                axes_F = axes_F, 
                                method.hv = method.hv, 
                                svmg = svmg, BAND = BAND,
    ...
    ##Current
    BAT_beak_obs_C <- int_null4(dat_ind4 = current_F, rn4 = "C", 
                                repn = rn, method = method, 
                                axes_F = axes_F, 
                                method.hv = method.hv, 
                                svmg = svmg, BAND = BAND,
    ...
    BAT_beak_obs_F <-  lapply(future_F, function(x){
      ...
      int_null4(dat_ind4 = future1_F, rn4 = "F", 
   ...
   ################################### 
    ...
    beak_r_obs[,2] <- c(hyper_disp_P, hyper_disp_H, 
      ...
      #Alpha hyper values (same code for main and beak analyses); take mean alpha of N replications
...
##################################################################################
##function to produce a plot for one set of species (All, Isl
...
hyper_plot <- function(beak_run_F, ggtitl = "a) All ", 
  ...
  beak_list <- data.table::rbindlist(beak_run_F[[2]], 
                                     fill=FALSE, idcol=NULL)
  beak_list$Type <- rep(c("P2H", "P2C", "H2C", "C2F"), 
                        (nrow(beak_list) / 4))
  ...
  obs_null2 <- mutate(obs_three, Type = c("P", "H", "C", "F")) 
  obs_null2$Type <- factor(obs_null2$Type, 
                           levels = c("P", "H", "C", "F"))
  if (method == "hyper" | method == "tree"){
...
if (inset_only){   
  ...
  if (method == "hyper" | method == "tree"){
    beak_run_F[[1]] <- cbind(rbind(beak_run_F[[1]][c("H", "C"),
                                                   c("Alpha", "Dispersion")], 
                                   beak_run_F[[1]][c("C", "F"),
                                                   c("Alpha", "Dispersion")]))
  } else if(method == "convex"){
    beak_run_F[[1]]$Alpha <- c(beak_run_F[[1]][c("H", "C"), "Alpha"], 
                               beak_run_F[[1]][c("C", "F"), "Alpha"])
  } 

  beak_run_F[[1]]$Type <- c("P2H", "P2C", "H2C", "C2F")
  beak_run_F[[1]]$Type <- factor(beak_run_F[[1]]$Type, 
                                 levels = c("P2C", "P2H", "H2C", 
                                            "C2F"))
...
contribution_own <- function(allSp_data = allSp2, 
  ...
  if (FD_PD == "FD_hyper"){
    ...
    k2Cont <- BAT::contribution(paCont2, tree2, relative = rel)
...
contr_tab <- function(allSp_data, FD_PD, rel = FALSE){
  ...
  #change EP to EPA (for anthropogenic)
  if (cont_p3$ExtinctionType[2] == "EP" & 
      cont_p3$ExtinctionType[7] == "EP"){
    cont_p3$ExtinctionType[c(2,7)] <- "EPA"
  } else{
    stop("Descending")
  }
  ...
  cont_p4_A <- filter(cont_p3, 
                    ExtinctionType %in% c("EH", "EPA", "EPU"))
...
PD_multi_format <- function(main_run_1){
  ...
  pmtr1 <- lapply(main_run_1[[1]], function(x){
    ...
    pmtr1c <- perc_change(c(x["P","Alpha"], 
                          x["F","Alpha"], 99, 99))[1]
  ...
  pmtr2_A <- matrix(ncol = 8,
                    nrow =  length(main_run_1[[3]]))
  pmtr2_D <- matrix(ncol = 8,
                    nrow =  length(main_run_1[[3]]))
  
  for (i in 1:nrow(pmtr2_A)){
    dumPMA <- filter(main_run_1[[3]][[i]],
                     Metric == "Alpha")
    pmtr2_A[i,1:4] <- as.numeric(dumPMA$Z)
    pmtr2_A[i,5:8] <- as.numeric(dumPMA$P)
    
    dumPMD <- filter(main_run_1[[3]][[i]],
                     Metric == "Dispersion")
    pmtr2_D[i,1:4] <- as.numeric(dumPMD$Z)
    pmtr2_D[i,5:8] <- as.numeric(dumPMD$P)
  }
  ...
  names(pmtr2_SS_A) <- c("PH_Alpha_Z_Med", "PH_Alpha_Z_RL",
                         "PH_Alpha_Z_RU",
                         "PC_Alpha_Z_Med", "PC_Alpha_Z_RL",
                         "PC_Alpha_Z_RU",
                         "HC_Alpha_Z_Med", "HC_Alpha_Z_RL",
                         "HC_Alpha_Z_RU",
                         "CF_Alpha_Z_Med", "CF_Alpha_Z_RL",
                         "CF_Alpha_Z_RU",
                         "PH_Alpha_P_Med", "PH_Alpha_P_RL",
                         "PH_Alpha_P_RU",
  ...
  names(pmtr2_SS_D) <- c("PH_Disp_Z_Med", "PH_Disp_Z_RL",
                         "PH_Disp_Z_RU",
                         "PC_Disp_Z_Med", "PC_Disp_Z_RL",
                         "PC_Disp_Z_RU",
                         "HC_Disp_Z_Med", "HC_Disp_Z_RL",
                         "HC_Disp_Z_RU",
                         "CF_Disp_Z_Med", "CF_Disp_Z_RL",
                         "CF_Disp_Z_RU",
                         "PH_Disp_P_Med", "PH_Disp_P_RL",
                         "PH_Disp_P_RU",
  ...
  pmtr3_A <- apply(pmtr2_A[,5:8], 2, function(y){
  ...
  names(pmtr3_A) <- c("P2H_P", "P2C_P", "H2C_P",
                      "C2F_P")
